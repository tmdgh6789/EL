...
function mark_start() {
    ...
    $("#msg2").text("");
    $("#cover").css("opacity", "1");
    $("#mark_button").text("press to mark");
    $("#mark_button").attr("class", "mark_button_normal");
    $("#control_button").css("visibility", 'hidden');
    ...
    switch (mark.state) {
        ...
        case mark.states.PLAY :
            ...
            if (id < player._duration) {
                ...
                $("#mark_position").css("margin-left", pos+7 + "px");
            }
...
function markbar_draw() {
    ...
    for (var i = 0; i < player._duration; i++) {
        ...
        total += each_width;
        $("#markbar").append("<div id='mark" + (i + 1) + "' class='mark_" + markbar_status[i]
...
$("#mark_button").mousedown(function() {
    if (mode == modes.MARK) {
        mark.mousedown = 1;
...
function marked_start(to_play=-1) {
    ...
    $("#mark_button").text("continue to mark");
    $("#mark_button").attr("class", "mark_button_continue");
}
...
function study_start() {
    ...
    $("#msg_study").text("");
    $("#cover").css("opacity", "1");
    $("#control_button").css("visibility", 'visible');
    $("#control_button").text("Skip");
...
function study_play_step() {
    ...
    if (study.steps_now == 1) {
        $("#msg1").text("");
        $("#msg_study").text(words.words);
        $("#msg_study").css("opacity", 0);
        $("#audio").empty();
        $("#audio").append('<audio id="audio_now"><source src="audio/' + words.audio
    ...
    $("#msg_study").css("opacity", opacity);

    study.audio_now = document.getElementById("audio_now");

    $("#audio_now").on('ended', function() {
        study.state = study.states.PLAY;
